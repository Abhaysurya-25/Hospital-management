...
async function getAllEntities(req, res) {
    ...
    try {
        if (type == 'user') {
            ...
            sql_query = `SELECT * FROM Data_entry_operator; `;
        }
        else if (type == 'front-desk') {
            sql_query = `SELECT * FROM Front_desk_operator; `;
        }
...
async function createEntity(req, res) {
    ...
    try {
        if (type == 'user') {
            ...
            sql_query = `INSERT INTO Affiliated_with(PhysicianID, DepartmentID) ` +
                `VALUES('${entity.physicianid}', ${entity.department});`;


        }
        else if (type == 'data-entry') {
            sql_query = `INSERT INTO Data_entry_operator(EmployeeID, Address) ` +
                `VALUES('${entity.employeeid}', '${entity.address}'); `;
        }
        else if (type == 'front-desk') {
            sql_query = `INSERT INTO Front_desk_operator(EmployeeID, Address) ` +
                `VALUES('${entity.employeeid}', '${entity.address}'); `;
        }
        else if (type == 'medication') {
